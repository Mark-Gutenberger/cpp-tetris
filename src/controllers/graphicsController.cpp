...
void GraphicsController::drawFigure(Figure* fig) {
  ...
  for (int n = 0; n < 4; n++) {
    for (int m = 0; m < 4; m++) {
      if (!fig->getForm()) exit(5);
      if (!fig->getForm()[n][m]) continue;
      int new_x = fig->getX() + m;
      int new_y = fig->getY() + n;

      if(new_x < 0 || new_x >= GAME_FIELD_WIDHT ||
        new_y < 0 || new_y >= GAME_FIELD_HEIGHT) {
        continue;
      }
...
void GraphicsController::displayGameEnd() {
  clear();
  printw("Game ended! You got %d points. Press enter to exit.", masterController->getGameController()->getScore());
  refresh();
  while (getch() != '\n') usleep(25000);
}