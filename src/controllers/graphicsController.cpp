...
void GraphicsController::drawUI(int score) {
  ...
  printw("[left|right] arrows - move");
  move(6, GAME_FIELD_WIDHT * 2 + 3);
  printw("[down] - speed up falling");
  move(7, GAME_FIELD_WIDHT * 2 + 3);
  printw("[x|<ctrl+c>] - exit the game");
}
...
void GraphicsController::drawFigure(Figure* fig) {
  ...
  for (int n = 0; n < 4; n++) {
	for (int m = 0; m < 4; m++) {
	  ...
	  mvaddch(GAME_FIELD_HEIGHT - new_y - 1, new_x * 2, '[');
	  mvaddch(GAME_FIELD_HEIGHT - new_y - 1, new_x * 2 + 1, ']');
	}
  }
