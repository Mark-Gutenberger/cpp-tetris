...
int **Figure::createDynamicFormFromStatic(int form[4][4]) {
  ...
  for (int i = 0; i < 4; i++) new_form[i] = new int[4];
...
bool Figure::checkCollision(const int **gameField) {
  for (int n = 0; n < 4; n++) {
    for (int m = 0; m < 4; m++) {
      ...
      if (gameField[field_y][field_x]) return true;
    }
  }
  return false;
}
...
int Figure::getX() { return this->x; }

int Figure::getY() { return this->y; }

int Figure::getColor() { return this->color; }

const int **Figure::getForm() { return (const int **)this->form; }
